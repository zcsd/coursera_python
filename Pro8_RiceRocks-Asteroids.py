...
def process_sprite_group(a_set, canvas):
    remove_set = set([])
    if(a_set == rock_group ):
        for a_sprite in a_set:
            a_sprite.update()
            a_sprite.draw(canvas)
    elif(a_set == missile_group or a_set == explosion_group):
        for a_sprite in a_set:
            if(a_sprite.update()):
                remove_set.add(a_sprite)
            else:
                a_sprite.update()
                a_sprite.draw(canvas)
...
def group_collide(group, other_object):
    ...
    for a_sprite in group:
        if(a_sprite.collide(other_object)):
            explosion_group.add(Sprite(a_sprite.pos, a_sprite.vel, 0, 0, explosion_image, explosion_info))
            remove_set.add(a_sprite)
    group.difference_update(remove_set)

    if(init_len > len(group)):
        return True
    else:
        return False
...
def group_group_collide(missile_group, rock_group):
    ...
    for a_missile in missile_group:
        if(group_collide(rock_group, a_missile)):
            remove_set.add(a_missile)
            counter += 1
...
class Sprite:
    ...
    def draw(self, canvas):
        if(self.animated == True):
                explosion_index = [(self.age) % 128, ((self.age) // 128) % 128]
